...
const Bar = () => (
  <div
    style={{
      height: '3px',
      background: `linear-gradient(90deg,${C},${M},${Y},${K})`,
      flexShrink: 0,
    }}
  />
);
...
const PLATES = {
  GTO: { mW: 356, mH: 508, rate: 750 },
  GTO52: { mW: 360, mH: 520, rate: 750 },
  KORD: { mW: 460, mH: 640, rate: 800 },
  Heidelberg: { mW: 480, mH: 650, rate: 900 },
};
const PAPERS = [
  { n: 'SRA3 12.60"×17.72"', w: 450, h: 320 },
  { n: 'A3 16.54"×11.69"', w: 420, h: 297 },
  { n: 'A4 11.69"×8.27"', w: 297, h: 210 },
  { n: '10.6"×8.33"', w: 269, h: 212 },
  { n: '8"×5"', w: 203, h: 127 },
  { n: '40"×25"', w: 1016, h: 635 },
  { n: 'SRA2', w: 640, h: 450 },
  { n: 'Custom', w: 0, h: 0 },
];
const COLS_OPTS = [
...
const m2i = (v) => parseFloat((v / 25.4).toFixed(3));
const i2m = (v) => Math.round(parseFloat(v || 0) * 25.4 * 100) / 100;
const fmt = (v) => Number(v).toLocaleString();
const cr = (s) => Math.ceil(s / 1000) * 1000;
const BIND_RATES = {
  Perfect: 1.5,
  'Saddle Stitch': 0.8,
  Spiral: 2.0,
...
const DEF_SHEET_PRESETS = [
  { l: 317.5, w: 254 },
  { l: 254, w: 158.75 },
  { l: 211.6, w: 203.2 },
  { l: 127, w: 203.2 },
]; // 12.5x10, 10x6.25, 8.33x8, 5x8 (inch→mm)
const DEF_LAMINATES = [
  { name: 'Gloss', rate: 0.03 },
  { name: 'Matt', rate: 0.04 },
  { name: 'Thermal Matt', rate: 0.06 },
];
const DEF_POSITIVE_RATE = 2.0; // Rs/sq.in per color (e.g. 4 Color → 4×2=8/sq.in)
const DEF_DIECUTTERS = [
  { id: 1, name: 'Kord', impRate: 1200 },
  { id: 2, name: 'Hand Platen', impRate: 650 },
];
...
const parseNum = (raw) => {
  ...
  if (mixed) {
    const w = parseFloat(mixed[1]),
      n = parseFloat(mixed[2]),
      d = parseFloat(mixed[3]);
    if (!d) return NaN;
    return w + (w < 0 ? -1 : 1) * (n / d);
  }
...
const NumField = ({ unit, mm, onMM, style, placeholder, readOnly }) => {
  ...
  const fmtVal = (v) =>
    unit === 'inch'
      ? (Math.round((v / 25.4) * 100) / 100).toFixed(2)
  ...
  return (
    <input
      value={buf !== null ? buf : fmtVal(mm)}
      onFocus={() => setBuf(fmtVal(mm))}
      onChange={(e) => {
        const raw = e.target.value;
        setBuf(raw);
        const n = parseNum(raw);
        if (!isNaN(n)) onMM(unit === 'inch' ? n * 25.4 : n);
      }}
      onBlur={() => setBuf(null)}
      style={style}
      placeholder={placeholder}
...
const FreeNum = ({ val, onVal, style, placeholder }) => {
  ...
  return (
    <input
      value={buf !== null ? buf : val === 0 ? '' : String(val)}
...
const Tog = ({ lbl, on, cb, col, sub }) => (
  <div
    ...
    style={{
      ...
      cursor: 'pointer',
      padding: '5px 8px',
      background: on ? col + '18' : BG,
      borderRadius: '7px',
      border: `1.5px solid ${on ? col : BD}`,
      flex: 1,
      minWidth: 0,
    }}
  >
    ...
    >
      {on && (
        <span style={{ color: '#fff', fontSize: '8px', lineHeight: 1 }}>✓</span>
      )}
    </div>
    <div style={{ flex: 1, minWidth: 0 }}>
      ...
      {sub && (
        <div style={{ fontSize: '8px', color: MT, marginTop: '1px' }}>
          {sub}
        </div>
      )}
    </div>
...
const CostRow = ({
  ...
  <div style={{ marginBottom: '6px' }}>
    ...
    >
      {lbl}
      {note && <span style={{ color: C, marginLeft: '4px' }}>{note}</span>}
    </div>
...
const OptCostRow = ({
  ...
  !enabled ? (
    ...
    <div style={{ marginBottom: '6px' }}>
      ...
      >
        <span style={{ fontSize: '9px', color: MT, fontWeight: '600' }}>
          {lbl}
        </span>
        <button
          onClick={() => {
...
const fitCalc = (pw, ph, grip, em, gap, jw, jh) => {
  const c1 = Math.floor((pw - 2 * em) / (jw + gap)),
    r1 = Math.floor((ph - grip - em) / (jh + gap));
  const u1 = Math.max(c1, 0) * Math.max(r1, 0);
  const c2 = Math.floor((pw - 2 * em) / (jh + gap)),
    r2 = Math.floor((ph - grip - em) / (jw + gap));
  const u2 = Math.max(c2, 0) * Math.max(r2, 0);
  return u2 > u1
    ? { cols: Math.max(c2, 0), rows: Math.max(r2, 0), ups: u2, rot: true }
...
const sheetsFromPaper = (pw, ph, sw, sh) => {
  const c1 = Math.floor(pw / sw),
    r1 = Math.floor(ph / sh),
    u1 = c1 * r1;
  const c2 = Math.floor(pw / sh),
    r2 = Math.floor(ph / sw),
    u2 = c2 * r2;
  return u2 > u1
    ? { cols: c2, rows: r2, ups: Math.max(u2, 1), rot: true }
...
const sigCalc = (sig, effQty) => {
  ...
  const isBB =
    sig.side === 'Back&Back - Left-Right' ||
    sig.side === 'Back&Back - Top-Bottom';
  ...
  const frontPages = sig.frontPages || 0,
    backPages = sig.backPages || 0;
  ...
  const totalPlates = isFB
    ? frontPlates + backPlates
    : groups.reduce((a, g) => a + g.plates, 0);
  ...
  const impressions = isFB
    ? frontImpressions + backImpressions
    : sheetsNeeded * totalPlates * (isBB ? 2 : 1);
...
export default function App() {
  ...
  const paperList = [
    ...settings.papers.map((p) => ({
      ...
      w: Math.max(p.l, p.w),
      h: Math.min(p.l, p.w),
      rate: p.packSheets ? p.packPrice / p.packSheets : 0,
    })),
    { n: 'Custom', w: 0, h: 0, rate: 0 },
  ];
  const [pi, setPi] = useState(0);
  const [cL, setCL2] = useState(450);
  const [cW2, setCW2] = useState(320);
  const pp = paperList[Math.min(pi, paperList.length - 1)];
  const isC = pp.n === 'Custom';
  ...
  const sheetL = isC ? pW : shOv ? shOv.l : pW,
    sheetH = isC ? pH : shOv ? shOv.h : pH;
  const curSheetL = isC ? cL : sheetL,
    curSheetH = isC ? cW2 : sheetH;
  const presetIdx = settings.sheetPresets.findIndex(
    (sp) =>
      Math.round(sp.l) === Math.round(curSheetL) &&
      Math.round(sp.w) === Math.round(curSheetH)
  );
  const sheetSizeSel = customMode
    ? 'Custom'
    : presetIdx >= 0
    ? String(presetIdx)
    : 'Custom';
  ...
  const pl = settings.plates.find((p) => p.name === platK) ||
    settings.plates[0] || { impRate: 750, grip: 38.1 };
  const [colS, setColS] = useState('4 Colors');
  const nc = nC(colS);
  const SIDE_OPTS = [
    'Single Side',
  ...
  const [fbColF, setFbColF] = useState('4 Colors');
  const [fbColB, setFbColB] = useState('1 Color');
  const [pGrip, setPGrip] = useState(13);
  const [jW, setJW] = useState(90);
  const [jH, setJH] = useState(55);
  ...
  // Laminate
  const [lamK, setLamK] = useState('Thermal Matt');
  const [lamCust, setLamCust] = useState(null);
  const [lamBoth, setLamBoth] = useState(false); // Laminate side — Both Side doubles the laminate cost
  const lRate =
    lamCust !== null
      ? lamCust
      : (settings.laminates.find((l) => l.name === lamK) || {}).rate || 0.06;
  ...
  const [positiveL, setPositiveL] = useState(0);
  const [positiveW, setPositiveW] = useState(0);
  ...
  const [msProdW, setMsProdW] = useState(210);
  const [msProdH, setMsProdH] = useState(297);
  ...
  const blankSig = (name) => ({
    ...
    pagesPerSheet: 4,
    side: 'Back&Back - Left-Right',
    plateRate: SETTINGS_PRINT_RATE,
    paperRate: 5,
    showPlan: false,
    ...PLAN_DEFAULTS,
    groups: [
  ...
  const addSig = () =>
    setMsSigs((s) => [...s, blankSig(`Paper ${s.length + 1}`)]);
  const rmSig = (id) =>
    setMsSigs((s) => (s.length > 1 ? s.filter((x) => x.id !== id) : s));
  const updSig = (id, patch) =>
    setMsSigs((s) => s.map((x) => (x.id === id ? { ...x, ...patch } : x)));
  const updGroup = (sigId, gi, patch) =>
    setMsSigs((s) =>
      s.map((x) =>
        x.id === sigId
          ? {
              ...x,
              groups: x.groups.map((g, i) =>
                i === gi ? { ...g, ...patch } : g
              ),
            }
          : x
  ...
  const addGroup = (sigId) =>
    setMsSigs((s) =>
      s.map((x) =>
        x.id === sigId
          ? { ...x, groups: [...x.groups, { colors: 1, pages: 0 }] }
          : x
      )
    );
  const rmGroup = (sigId, gi) =>
    setMsSigs((s) =>
      s.map((x) =>
        x.id === sigId
          ? { ...x, groups: x.groups.filter((_, i) => i !== gi) }
          : x
      )
    );
  ...
  const mPW = sheetL - 2 * eM;
  const botG = (bbTB && both) || sbsOn ? pGrip : eM;
  const mPH = sheetH - pGrip - botG;
  const sW = mPW - 20,
    sH = mPH - 20;
  const lW = cols * jW + (cols > 1 ? (cols - 1) * eGH : 0);
  const lH = rows * jH + (rows > 1 ? (rows - 1) * eGV : 0);
  const bbA = bbLR || bbTB;
  const ups = cols * rows;
  ...
  const platC = ePl * (pl.impRate / 1000) * chSh;
  const papC = nPap * pRate;
  const printImpCost = (imp / 1000) * pl.impRate; // Printing(Impression) cost — separate from Plate cost
  const lamJW = lW + 2 * cmSz + 1,
    lamJH = lH + 2 * cmSz + 1;
  const lamSq = m2i(lamJW) * m2i(lamJH);
  const lamC =
    (lamK === 'No Laminate' ? 0 : lamSq * lRate * sheetsNeed) *
    (lamBoth ? 2 : 1);
  const printCost =
    platC + papC + (enabledCosts.laminate ? lamC : 0) + printImpCost;
  const extraCost =
    artwork +
    planning +
    (enabledCosts.positive ? positive : 0) +
    cutting +
    foilBlock +
    foiling +
    dieCutter +
    ...
    gathering +
    binding +
    (enabledCosts.rimming ? rimming : 0) +
    addMat +
    transport +
    stripping +
    numbering +
  ...
  const msEffQty = msMultiCopies
    ? msQty * Math.max(msCopies, 1) * (Math.max(msDup, 0) + 1)
    : msQty;
  const msCalcs = msSigs.map((s) => {
    const livePaper = s.paperCustomMode
      ? null
      : settings.papers.find((p) => p.name === s.paperName);
    const livePlate = settings.plates.find(
      (p) => p.name === (s.plateName || settings.plates[0]?.name)
    );
    const liveRates = {
      paperRate:
        livePaper && livePaper.packSheets
          ? livePaper.packPrice / livePaper.packSheets
          : s.paperRate,
  ...
  const msPrintingCost = msCalcs.reduce(
    (a, s) => a + s.totalPlates * (s.plateRate || 0),
    0
  );
  ...
  const cutOpts = (() => {
    ...
    for (let dc = 1; dc <= 8; dc++)
      for (let dr = 1; dr <= 8; dr++) {
        const cw = sheetL / dc,
          ch = sheetH / dr;
        const uc = Math.floor((cw - 2 * eM) / (jW + eGH)),
          ur = Math.floor((ch - pGrip - eM) / (jH + eGV));
        if (uc > 0 && ur > 0) {
          const u = uc * ur;
          o.push({ u, uc, ur, sh: cr(Math.ceil(qty / u)) });
        }
      }
  ...
  useEffect(() => {
    if (
      !colsTouched &&
      bestOpt &&
      (bestOpt.uc !== cols || bestOpt.ur !== rows)
    ) {
      setCols(bestOpt.uc);
      setRows(bestOpt.ur);
    }
  ...
  const CX = 288,
    CY = 210,
    sc = Math.min(CX / sheetL, CY / sheetH);
  const dW = Math.round(sheetL * sc),
    dH = Math.round(sheetH * sc);
  const gPx = Math.round(pGrip * sc),
    bGpx = (bbTB && both) || sbsOn ? gPx : Math.round(eM * sc);
  const ePx = Math.round(eM * sc);
  const jWp = Math.round(jW * sc),
    jHp = Math.round(jH * sc);
  const gHp = Math.round(eGH * sc),
    gVp = Math.round(eGV * sc);
  ...
  const rotateSheet = () => {
    if (isC) {
      ...
      setCL2(cW2);
      setCW2(t);
    } else setShOv({ l: sheetH, h: sheetL });
  };
  const showWarn = (msg) => {
    setWarn(msg);
  ...
  const MiniCanvas = ({ pw, ph, jw, jh, cols, rows, col }) => {
    const CX = 104,
      CY = 76,
      sc = Math.min(CX / pw, CY / ph);
    const dw = Math.max(Math.round(pw * sc), 20),
      dh = Math.max(Math.round(ph * sc), 20);
    const gpx = Math.round(pGrip * sc),
      epx = Math.round(eM * sc);
    const jwp = Math.max(Math.round(jw * sc), 2),
      jhp = Math.max(Math.round(jh * sc), 2);
    const gapx = Math.round(eGH * sc);
    const lwp = cols * jwp + (cols > 1 ? (cols - 1) * gapx : 0),
      lhp = rows * jhp + (rows > 1 ? (rows - 1) * gapx : 0);
  ...
  const SigCanvas = ({ pw, ph, jw, jh, cols, rows, col }) => {
    const CX = 104,
      CY = 76,
      sc = Math.min(CX / Math.max(pw, 1), CY / Math.max(ph, 1));
    const dw = Math.max(Math.round(pw * sc), 20),
      dh = Math.max(Math.round(ph * sc), 20);
    const gpx = Math.round(10 * sc),
      epx = Math.round(5 * sc);
    const jwp = Math.max(Math.round(jw * sc), 2),
      jhp = Math.max(Math.round(jh * sc), 2);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ background: K, flexShrink: 0 }}>
          ...
          {screen === 'plan' ? (
            <>
                ...
                >
                  {['mm', 'inch'].map((u) => (
                    <button
                      ...
                      onClick={() => setUnit(u)}
                      style={{
                        background: unit === u ? C : 'transparent',
                        border: 'none',
                        borderRadius: '4px',
                        padding: '3px 6px',
                        color: '#fff',
              ...
              <div
                style={{ display: 'flex', padding: '0 10px 8px', gap: '2px' }}
              >
                {[
                  ['single', '📄 Single Sheet'],
                  ['multi', '📚 Multiple Sheets'],
                ].map(([k, l]) => (
                  <button
                    key={k}
                    ...
                    style={{
                      ...
                      borderRadius: '6px',
                      background:
                        activeTab === k ? C : 'rgba(255,255,255,0.12)',
                      color: '#fff',
                      fontWeight: '700',
                      fontSize: '10px',
                      cursor: 'pointer',
        ...
        {screen === 'plan' && (
          ...
          >
            {activeTab === 'single' && (
              <>
                ...
                >
                  ...
                  >
                    <div style={{ flex: '1.2', minWidth: 0 }}>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          setPi(
                            paperList.findIndex((p) => p.n === e.target.value)
                          );
                          setShOv(null);
                          setCustomMode(false);
                        }}
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          setCustomMode(false);
                          const preset =
                            settings.sheetPresets[Number(e.target.value)];
                          if (!preset) return;
                          if (isC) {
                            setCL2(preset.l);
                            setCW2(preset.w);
                          } else setShOv({ l: preset.l, h: preset.w });
                        }}
                        style={{
                          width: '100%',
                          padding: '5px 3px',
                    ...
                    >
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <NumField
                          ...
                          onMM={(v) => {
                            if (isC) setCL2(v);
                            else setShOv({ l: v, h: shOv ? shOv.h : pH });
                          }}
                          style={{
                            width: '100%',
                            padding: '5px 3px',
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <NumField
                          ...
                          onMM={(v) => {
                            if (isC) setCW2(v);
                            else setShOv({ l: shOv ? shOv.l : pW, h: v });
                          }}
                          style={{
                            width: '100%',
                            padding: '5px 3px',
                  ...
                  {(sheetExceedsPaper || jobExceedsPaper) && (
                    <div
                      style={{
                        background: '#FEE2E2',
                        borderRadius: '6px',
                    ...
                    >
                      ⚠ {sheetExceedsPaper ? 'Sheet size' : 'Job size'} must be
                      smaller than the paper size ({fs(pW)}×{fs(pH)}).
                    </div>
                  )}
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <select
                        value={platK}
                        onChange={(e) => setPlatK(e.target.value)}
                        style={{
                          width: '100%',
                          padding: '5px 3px',
                          border: `1.5px solid ${BD}`,
                    ...
                    <div style={{ flex: '1.6', minWidth: 0 }}>
                      ...
                      <select
                        value={sideMode}
                        onChange={(e) => chgSideMode(e.target.value)}
                        style={{
                          width: '100%',
                          padding: '5px 3px',
                          border: `1.5px solid ${BD}`,
                  ...
                  {isFB && (
                    ...
                    >
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <select
                          value={fbColF}
                          onChange={(e) => setFbColF(e.target.value)}
                          style={{
                            width: '100%',
                            padding: '5px 3px',
                            border: `1.5px solid ${C}`,
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <select
                          value={fbColB}
                          onChange={(e) => setFbColB(e.target.value)}
                          style={{
                            width: '100%',
                            padding: '5px 3px',
                      ...
                      >
                        ...
                        <div
                          style={{
                            fontSize: '13px',
                            fontWeight: '800',
                            color: cols === opt.uc && rows === opt.ur ? C : K,
                            lineHeight: 1,
                          }}
                        >
                          {opt.u}
                        </div>
                        <div style={{ fontSize: '7px', color: MT }}>
                          ups/sheet
                        </div>
                        <div
                          style={{
                ...
                >
                  ...
                  >
                    <span
                      style={{ fontSize: '10px', fontWeight: '700', color: T }}
                    >
                      Planning Sheet
                    </span>
                    <span style={{ fontSize: '9px', color: MT }}>
                      {showPlan ? '▾' : '▸'}
                    </span>
                  </button>
                  {showPlan && (
                  ...
                  {showPlan && (
                    <>
                      ...
                      >
                        {[
                          ['Column Space', gH, (v) => setGH(v), MT],
                          ['Row Space', gV, (v) => setGV(v), MT],
                          ['Cut Mark', cmSz, setCmSz, K],
                        ].map(([l, v, s, col]) => (
                        ...
                        <button
                          onClick={() => setCm(!cm)}
                          title="Toggle cut marks on canvas"
                          style={{
                            flexShrink: 0,
                            background: cm ? K : BG,
                            border: `1.5px solid ${cm ? K : BD}`,
                            borderRadius: '5px',
                            padding: '4px 7px',
                            cursor: 'pointer',
                            color: cm ? '#fff' : MT,
                      ...
                      <div
                        style={{ fontSize: '7px', color: MT, marginTop: '3px' }}
                      >
                        Plate Grip:
                        <NumField
                          unit={unit}
                      ...
                      {lW > mPW || lH > mPH ? (
                        ...
                        >
                          ⚠️ Exceeds print area {fs(lW)}×{fs(lH)}&gt;{fs(mPW)}×
                          {fs(mPH)}
                        </div>
                      ) : null}
                      ...
                      >
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          >
                            <div
                              style={{ height: '1px', flex: 1, background: C }}
                            />
                            <span
                              style={{
                                fontSize: '6px',
                          ...
                          >
                            ...
                            >
                              <button
                                onClick={rotateSheet}
                                title="Rotate sheet (swaps Length/Width)"
                                style={{
                                  width: '14px',
                                  height: '11px',
                                  padding: 0,
                              ...
                              <div
                                ...
                                style={{
                                  width: '11px',
                                  height: '11px',
                                  border: `1.5px solid ${safe ? T : BD}`,
                                  borderRadius: '2px',
                                  background: safe ? T : '#fff',
                                  cursor: 'pointer',
                                  display: 'flex',
                                  alignItems: 'center',
                                  justifyContent: 'center',
                            ...
                            {safe && (
                              <div
                                style={{
                                  ...
                                  top: `${gPx + sfO}px`,
                                  left: `${ePx + sfO}px`,
                                  width: `${dW - 2 * ePx - 2 * sfO}px`,
                                  height: `${dH - gPx - bGpx - 2 * sfO}px`,
                                ...
                                (() => {
                                  ...
                                  return (
                                    <rect
                                      x={x + 2}
                                      y={y + 2}
                                      width={Math.max(jWp - 4, 4)}
                                      height={Math.max(jHp - 4, 4)}
                                      fill="none"
                                      stroke="#7C3AED"
                                      strokeWidth="1"
                                      strokeDasharray="2,2"
                            ...
                            {allU.map((u, idx) => {
                              const bg = [CL, ML, '#FFFBE0', '#F0FFF4'][
                                  idx % 4
                                ],
                                bd = [C, M, '#D4A000', SUC][idx % 4];
                              if (u.x + jWp > dW + 2 || u.y + jHp > dH + 2)
                                return null;
                              return (
                                <div
                                  key={u.id}
                          ...
                          >
                            <span style={{ fontSize: '7px', color: MT }}>
                              space {gH > 0 ? fs(gH) : 'none'}
                            </span>
                            {cutterFile && (
                              <span
                                style={{
                      ...
                      {bbA && (
                        ...
                        >
                          B+B: {fmt(aSh)} actual sh → {fmt(chSh)} charged →{' '}
                          {fmt(imp)} imp
                        </div>
                      )}
                ...
                >
                  ...
                  >
                    <Tog
                      ...
                      on={msMultiSheet}
                      cb={() => {
                        setMsMultiSheet(!msMultiSheet);
                        if (!msMultiSheet) setMsMultiCopies(false);
                      }}
                      col="#7C3AED"
                    />
                    <Tog
                      ...
                      on={msMultiCopies}
                      cb={() => {
                        setMsMultiCopies(!msMultiCopies);
                        if (!msMultiCopies) setMsMultiSheet(false);
                      }}
                      col="#7C3AED"
                    />
                  </div>
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <input
                        value={msProduct}
                        onChange={(e) => setMsProduct(e.target.value)}
                        list="ms-product-presets"
                        style={{
                          width: '100%',
                          padding: '5px 6px',
                          border: `1.5px solid ${BD}`,
                      ...
                      />
                      <datalist id="ms-product-presets">
                        {PRODUCT_PRESETS.map((p) => (
                          <option key={p} value={p} />
                        ))}
                      </datalist>
                  ...
                  <div
                    style={{
                      display: 'flex',
                      gap: '4px',
                      marginBottom: msMultiCopies ? '5px' : 0,
                    }}
                  >
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <FreeNum
                        ...
                        onVal={(v) => {
                          ...
                          if (n > msSigs.length) {
                            const add = [];
                            for (let i = msSigs.length; i < n; i++)
                              add.push(blankSig(`Paper ${i + 1}`));
                            setMsSigs((s) => [...s, ...add]);
                          } else if (n < msSigs.length) {
                            setMsSigs((s) => s.slice(0, n));
                          }
                        }}
                        style={{
                          width: '100%',
                  ...
                  {msMultiCopies && (
                    <div style={{ display: 'flex', gap: '4px' }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <FreeNum
                          val={msCopies}
                          onVal={(v) => setMsCopies(Math.max(v, 1))}
                          style={{
                            width: '100%',
                            padding: '5px 3px',
                    ...
                    >
                      ...
                      >
                        <input
                          value={s.name}
                          onChange={(e) =>
                            updSig(s.id, { name: e.target.value })
                          }
                          style={{
                            border: 'none',
                        ...
                        <div style={{ display: 'flex', gap: '4px' }}>
                          <button
                            onClick={() =>
                              updSig(s.id, { minimized: !s.minimized })
                            }
                            style={{
                              background: BG,
                              border: `1px solid ${BD}`,
                        ...
                        <>
                          ...
                          >
                            <div style={{ flex: '1.3', minWidth: 0 }}>
                              ...
                              <select
                                value={
                                  s.paperCustomMode
                                    ? 'Custom'
                                    : s.paperName || paperList[0]?.n
                                }
                                onChange={(e) => {
                                  if (e.target.value === 'Custom') {
                                    updSig(s.id, { paperCustomMode: true });
                                    return;
                                  }
                                  const p = paperList.find(
                                    (x) => x.n === e.target.value
                                  );
                                  if (p)
                                    updSig(s.id, {
                            ...
                            <button
                              ...
                              title="Reset sheet to paper size"
                              style={{
                                background: BG,
                                border: `1px solid ${BD}`,
                                borderRadius: '5px',
                          ...
                          >
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              ...
                              <select
                                value={s.plateName || settings.plates[0]?.name}
                                onChange={(e) => {
                                  const p = settings.plates.find(
                                    (x) => x.name === e.target.value
                                  );
                                  updSig(s.id, {
                                    plateName: e.target.value,
                                    plateRate: p ? p.impRate : s.plateRate,
                                  });
                                }}
                                style={{
                                  width: '100%',
                            ...
                            <button
                              onClick={() =>
                                updSig(s.id, { sbsPlate: !s.sbsPlate })
                              }
                              ...
                              style={{
                                ...
                                height: '24px',
                                flexShrink: 0,
                                background: s.sbsPlate ? C : BG,
                                border: `1.5px solid ${s.sbsPlate ? C : BD}`,
                                borderRadius: '5px',
                                cursor: 'pointer',
                                color: s.sbsPlate ? '#fff' : MT,
                                fontWeight: '800',
                                fontSize: '11px',
                              }}
                            >
                            ...
                            <div style={{ flex: '1.5', minWidth: 0 }}>
                              ...
                              <select
                                value={s.side}
                                onChange={(e) =>
                                  updSig(s.id, { side: e.target.value })
                                }
                                style={{
                                  width: '100%',
                                  padding: '4px 2px',
                              ...
                              >
                                ...
                                <option>Back&Back - Left-Right</option>
                                <option>Front&Back</option>
                              </select>
                            </div>
                          </div>
                          {(() => {
                            const pl2 = settings.plates.find(
                              (p) =>
                                p.name ===
                                (s.plateName || settings.plates[0]?.name)
                            );
                            if (!pl2) return null;
                            const exceeds =
                              s.sheetW > pl2.maxL || s.sheetH > pl2.maxW;
                            return exceeds ? (
                              <div
                                style={{
                                  background: '#FEE2E2',
                              ...
                              >
                                ⚠ Sheet size exceeds {pl2.name}'s max sheet size
                                ({fs(pl2.maxL)}×{fs(pl2.maxW)}).
                              </div>
                            ) : null;
                          })()}
                          {s.side === 'Front&Back' && (
                            ...
                            >
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <FreeNum
                                  val={s.frontPages || 0}
                                  onVal={(v) => updSig(s.id, { frontPages: v })}
                                  style={{
                                    width: '100%',
                                    padding: '4px 2px',
                                    border: `1.5px solid ${C}`,
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <select
                                  value={s.frontColors || 4}
                                  onChange={(e) =>
                                    updSig(s.id, {
                                      frontColors: Number(e.target.value),
                                    })
                                  }
                                  style={{
                                    width: '100%',
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <FreeNum
                                  val={s.backPages || 0}
                                  onVal={(v) => updSig(s.id, { backPages: v })}
                                  style={{
                                    width: '100%',
                                    padding: '4px 2px',
                                    border: `1.5px solid ${M}`,
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <select
                                  value={s.backColors || 1}
                                  onChange={(e) =>
                                    updSig(s.id, {
                                      backColors: Number(e.target.value),
                                    })
                                  }
                                  style={{
                                    width: '100%',
                          ...
                          {s.side === 'Front&Back' &&
                            (s.frontPages || 0) !== s.groupPages && (
                              ...
                              >
                                ⚠ Front pages ({fmt(s.frontPages || 0)}) doesn't
                                tally with total pages below (
                                {fmt(s.groupPages)}).
                              </div>
                            )}
                          ...
                          {s.groups.map((g, gi) => (
                            ...
                            >
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                >
                                  No of {g.colors} color pages
                                </div>
                                <FreeNum
                                  val={g.pages}
                                  onVal={(v) =>
                                    updGroup(s.id, gi, { pages: v })
                                  }
                                  style={{
                                    width: '100%',
                                    padding: '4px 2px',
                          ...
                          >
                            ...
                            >
                              {s.totalPlates}plates × Rs.{SETTINGS_PRINT_RATE}
                              /plate (rate from Settings)
                            </div>
                          </div>
                          ...
                          <button
                            onClick={() =>
                              updSig(s.id, { showPlan: !s.showPlan })
                            }
                            style={{
                              width: '100%',
                              marginTop: '5px',
                          ...
                          >
                            ...
                            >
                              {s.showPlan ? '▾' : '▸'} Planning Sheet —{' '}
                              {s.fit.cols}×{s.fit.rows} = {s.perPaper}{' '}
                              sheets/paper{s.fit.rot ? ' ↺' : ''}
                            </span>
                          </button>
                          {s.showPlan && (
                            <div
                            ...
                            >
                              ...
                              >
                                <div>
                                  Paper: {fs(s.paperW)}×{fs(s.paperH)}
                                </div>
                                <div>
                                  Sheet: {fs(s.sheetW)}×{fs(s.sheetH)}
                                </div>
                                <div style={{ color: tCol, fontWeight: '700' }}>
                                  {s.fit.cols}×{s.fit.rows} = {s.perPaper}{' '}
                                  sheets/paper{s.fit.rot ? ' ↺' : ''}
                                </div>
                              </div>
            ...
            >
              ...
              >
                <CostRow lbl="Artwork" val={artwork} set={setArtwork} />
                <CostRow lbl="Planning" val={planning} set={setPlanning} />
              </div>
              ...
              {activeTab === 'single' ? (
                <>
                  ...
                  >
                    <div
                      style={{ fontSize: '10px', color: T, fontWeight: '700' }}
                    >
                      Paper ● — {fmt(nPap)} papers
                    </div>
                    <div
                      style={{ fontSize: '12px', fontWeight: '800', color: T }}
                    >
                      Rs.{Math.round(papC).toLocaleString()}
                    </div>
                  </div>
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        Rs.{Math.round(platC).toLocaleString()}
                      </div>
                    </div>
                    <div
                      style={{
                    ...
                    >
                      <button
                        ...
                        style={{
                          ...
                          flexShrink: 0,
                          padding: 0,
                          border: `1.5px solid ${sbs ? T : BD}`,
                          borderRadius: '3px',
                          background: sbs ? T : '#fff',
                          color: '#fff',
                          fontSize: '9px',
                          cursor: 'pointer',
                          display: 'flex',
                    ...
                    {sbs && !sbsOK && (
                      <div style={{ marginTop: '4px' }}>
                        ...
                        >
                          ⚠ Sheet exceeds half the plate's print area for
                          side-by-side.
                        </div>
                      </div>
                    )}
                  ...
                  >
                    <div
                      style={{ fontSize: '10px', color: T, fontWeight: '700' }}
                    >
                      Printing (Impression) ● — {fmt(imp)} imp
                    </div>
                    <div
                      style={{ fontSize: '12px', fontWeight: '800', color: T }}
                    >
                      Rs.{Math.round(printImpCost).toLocaleString()}
                    </div>
                  </div>
                  <div
                    style={{
                  ...
                  >
                    ...
                    >
                      ...
                      {bbA && (
                        <span style={{ color: M }}> ×2={fmt(aSh * 2)}</span>
                      )}
                      {' → '}
                      {fmt(chSh)} × {effNc}C{' = '}
                    ...
                    {(bbA ? aSh * 2 : aSh) !== chSh && (
                      ...
                      >
                        ⚠ {fmt(bbA ? aSh * 2 : aSh)} → UP to {fmt(chSh)}
                      </div>
                    )}
                  </div>
                </>
               ...
               different plate/rate, so a single mixed total would misreport the real cost. */
                <div style={{ marginBottom: '6px' }}>
                  {msCalcs.map((s, i) => {
                    const sPrintCost = s.totalPlates * (s.plateRate || 0);
                        ...
                        {isFBSig ? (
                          <>
                            ...
                            >
                              ...
                              >
                                Front — {fmt(s.frontImpressions)} imp
                              </span>
                              <span
                                style={{
                                  fontSize: '10px',
                            ...
                            >
                              ...
                              >
                                Back — {fmt(s.backImpressions)} imp
                              </span>
                              <span
                                style={{
                                  fontSize: '10px',
                            ...
                            >
                              ...
                              >
                                Printing (Impression) ● — {fmt(s.impressions)}{' '}
                                imp total
                              </span>
                              <span
                                style={{
                          ...
                          >
                            ...
                            >
                              Printing (Impression) ● — {fmt(s.impressions)} imp
                            </span>
                            <span
                              style={{
                                fontSize: '11px',
              ...
              {(() => {
                const defs = [
                  ...
                  ...(activeTab === 'single'
                    ? [{ k: 'laminate', l: 'Laminating', special: 'laminate' }]
                    : []),
                  {
                    k: 'dieCutter',
                    l: 'Die Cutter',
                  ...
                  },
                  {
                    ...
                    set: setDieCutImp,
                    hasDD: true,
                    ddOpts: settings.dieCutters.map((d) => d.name),
                    ddKey: dieCutMachine,
                    setDdk: setDieCutMachine,
                  },
                  {
                    k: 'foilBlock',
                    l: 'Foil Block',
                  ...
                  <>
                          ...
                          return (
                            <div key="laminate" style={{ marginBottom: '6px' }}>
                              ...
                              >
                                ...
                                >
                                  {[
                                    ...settings.laminates.map((l) => l.name),
                                    'No Laminate',
                                  ].map((n) => (
                                ...
                                >
                                  Rs.{Math.round(lamC).toLocaleString()}
                                </div>
                              </div>
                              <div
                                style={{
                              ...
                              {lamK !== 'No Laminate' && (
                                ...
                                >
                                  {fs(lamJW)}×{fs(lamJH)} ={' '}
                                  {(Math.round(lamSq * 100) / 100).toFixed(2)}{' '}
                                  sq.in × Rs.{lRate} × {nPap}sh
                                  {lamBoth ? ' × 2 (both side)' : ''}
                                </div>
                              )}
                            </div>
                          );
                        if (d.special === 'positive')
                          return (
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                Rs.{Math.round(d.val).toLocaleString()}
                              </span>
                              <button
                                onClick={() =>
                                  setFieldDialog({ ...d, val: d.val })
                                }
                                style={{
                                  background: 'none',
                                  border: 'none',
                    ...
                    {notEnabled.length > 0 && (
                      <select
                        value=""
                        onChange={(e) => {
                          const d = notEnabled.find(
                            (x) => x.k === e.target.value
                          );
              ...
              {fieldDialog && (
                ...
                >
                  ...
                  >
                    ...
                    {fieldDialog.hasDD && (
                      <select
                        value={fieldDialog.ddKey}
                        onChange={(e) => {
                          fieldDialog.setDdk(e.target.value);
                          setFieldDialog({
                            ...fieldDialog,
                            ddKey: e.target.value,
                          });
                        }}
                        style={{
                          width: '100%',
                    ...
                    <FreeNum
                      val={fieldDialog.val}
                      onVal={(v) => setFieldDialog({ ...fieldDialog, val: v })}
                      style={{
                        width: '100%',
                        padding: '8px',
                        border: `1.5px solid ${C}`,
                    ...
                    <div style={{ display: 'flex', gap: '6px' }}>
                      ...
                      <button
                        onClick={() => {
                          fieldDialog.set(fieldDialog.val);
                          toggleCost(fieldDialog.k, true);
                          setFieldDialog(null);
                        }}
                        style={{
                          flex: 1,
              ...
              {activeTab === 'single' && bk && sheets.length > 0 && (
                ...
                >
                  ...
                  {sheets.map((s, i) => (
                    ...
                    >
                      <span>
                        {s.type} — {s.colors} — {s.pages}pp — {s.paper}
                      </span>
                      <span style={{ fontWeight: '700' }}>
                        Rs.{s.cost.toLocaleString()}
                      </span>
                    </div>
                  ))}
                </div>
              ...
              {[
                ...
                >
                  <span
                    style={{ fontSize: '10px', color: MT, fontWeight: '600' }}
                  >
                    {l}
                  </span>
                  <span
                    style={{ fontSize: '11px', fontWeight: '700', color: T }}
                  >
              ...
              >
                ...
                >
                  <div>
                    ...
                    <div
                      style={{ color: C, fontSize: '10px', fontWeight: '700' }}
                    >
                      Rs.
                      {(total / (activeTab === 'single' ? qty : msQty)).toFixed(
                        2
                      )}
                      /pc
                    </div>
                  ...
                  <span
                    style={{ color: Y, fontWeight: '800', fontSize: '19px' }}
                  >
            ...
            {activeTab === 'single' && bk && (
              ...
              >
                ...
                {sheets.length === 0 && (
                  ...
                  >
                    No sheets added yet. Tap + to add cover, text or photo
                    pages.
                  </div>
                )}
                {sheets.map((s, i) => {
                  const tCol = ['Cover Page', 'Back Cover'].includes(s.type)
                    ? C
                    : s.type === 'Photo Pages'
                    ? Y
                    : M;
                  return (
                    ...
                    >
                      <MiniCanvas
                        pw={sheetL}
                        ph={sheetH}
                        jw={s.rot ? s.pageH : s.pageW}
                        jh={s.rot ? s.pageW : s.pageH}
                        cols={s.cols}
                        rows={s.rows}
                        col={tCol}
                      />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        >
                          ...
                          <button
                            onClick={() =>
                              setSheets((sh) => sh.filter((x) => x.id !== s.id))
                            }
                            style={{
                              background: '#FEE2E2',
                              border: 'none',
            ...
            <div style={{ padding: '7px 8px 80px' }}>
              ...
              >
                ...
                >
                  {saved
                    ? '✓ Saved — Return to Quotation'
                    : 'Save & Apply to Quotation →'}
                </div>
              </button>
            </div>
          </div>
          ...
          >
            ...
            >
              ...
              {settings.papers.map((p) => (
                ...
                >
                  ...
                  <div style={{ fontSize: '7px', color: MT, marginTop: '2px' }}>
                    Rs/sheet:{' '}
                    {p.packSheets
                      ? (p.packPrice / p.packSheets).toFixed(2)
                      : '0.00'}
                  </div>
                </div>
              ))}
            ...
            >
              ...
              {settings.plates.map((p) => (
                ...
                >
                  ...
                  >
                    ...
                    <button
                      onClick={() =>
                        setSettings((s) => ({
                          ...s,
                          plates: s.plates.filter((x) => x.id !== p.id),
                        }))
                      }
                      style={{
                        background: 'none',
                  ...
                  >
                    {[
                      ['Max L', p.maxL, (v) => ({ maxL: v }), true],
                      ['Max W', p.maxW, (v) => ({ maxW: v }), true],
                      ['Print L', p.printL, (v) => ({ printL: v }), true],
                      ['Print W', p.printW, (v) => ({ printW: v }), true],
                      ['Grip', p.grip, (v) => ({ grip: v }), true],
                      ['Imp/1000', p.impRate, (v) => ({ impRate: v }), false],
                    ].map(([lbl, val, patch, isSize]) => (
            ...
            >
              ...
              {settings.sheetPresets.map((sp, i) => (
                ...
                >
                  <span style={{ fontSize: '9px', color: MT, flexShrink: 0 }}>
                    L
                  </span>
                  <NumField
                    unit={unit}
                    ...
                    onMM={(v) =>
                      setSettings((s) => ({
                        ...s,
                        sheetPresets: s.sheetPresets.map((x, j) =>
                          j === i ? { ...x, l: v } : x
                        ),
                      }))
                    }
                  ...
                  <button
                    onClick={() =>
                      setSettings((s) => ({
                        ...s,
                        sheetPresets: s.sheetPresets.filter((_, j) => j !== i),
                      }))
                    }
                    style={{
                      background: 'none',
              ...
              <button
                onClick={() =>
                  setSettings((s) => ({
                    ...s,
                    sheetPresets: [...s.sheetPresets, { l: 200, w: 150 }],
                  }))
                }
                style={{
                  width: '100%',
                ...
                >
                  ...
                  <button
                    onClick={() =>
                      setSettings((s) => ({
                        ...s,
                        laminates: s.laminates.filter((_, j) => j !== i),
                      }))
                    }
                    style={{
                      background: 'none',
            ...
            >
              ...
              >
                Positive (Rs/sq.in per color)
              </div>
              <FreeNum
                val={settings.positiveRate}
                onVal={(v) => setSettings((s) => ({ ...s, positiveRate: v }))}
                style={{
                  width: '100px',
                  padding: '5px',
                  border: `1px solid ${BD}`,
              ...
              />
              <div style={{ fontSize: '7px', color: MT, marginTop: '3px' }}>
                e.g. 4 Color → {(settings.positiveRate * 4).toFixed(2)}/sq.in
              </div>
            </div>
            ...
            >
              ...
              {settings.dieCutters.map((d) => (
                ...
                >
                  ...
                  />
                  <span style={{ fontSize: '7px', color: MT }}>Imp/1000</span>
                  <FreeNum
                    val={d.impRate}
                    onVal={(v) =>
                      setSettings((s) => ({
                  ...
                  <button
                    onClick={() =>
                      setSettings((s) => ({
                        ...s,
                        dieCutters: s.dieCutters.filter((x) => x.id !== d.id),
                      }))
                    }
                    style={{
                      background: 'none',
              ...
              <button
                onClick={() =>
                  setSettings((s) => ({
                    ...
                    dieCutters: [
                      ...s.dieCutters,
                      { id: Date.now(), name: 'New Machine', impRate: 700 },
                    ],
                  }))
                }
                style={{
            ...
            >
              ...
              <FreeNum
                val={settings.rimmingRate}
                onVal={(v) => setSettings((s) => ({ ...s, rimmingRate: v }))}
                style={{
                  width: '100px',
                  padding: '5px',
                  border: `1px solid ${BD}`,
            ...
            >
                ...
                >
                  ...
                  <button
                    onClick={() =>
                      setSettings((s) => ({
                        ...s,
                        products: s.products.filter((_, j) => j !== i),
                      }))
                    }
                    style={{
                      background: 'none',
              ...
              <button
                onClick={() =>
                  setSettings((s) => ({
                    ...s,
                    products: [...s.products, 'New Product'],
                  }))
                }
                style={{
                  width: '100%',
        ...
        {screen === 'plan' && activeTab === 'single' && bk && (
          <button
            ...
            style={{
              ...
              fontWeight: '300',
              cursor: 'pointer',
              boxShadow: '0 4px 12px rgba(212,0,110,0.4)',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              zIndex: 10,
          ...
          >
            ...
            >
              ...
              {[
                {
                  ...
                  type: 'sel',
                  val: newSheet.type,
                  set: (v) => setNewSheet({ ...newSheet, type: v }),
                  opts: [
                    'Cover Page',
                    'Text Pages',
                    'Photo Pages',
                ...
                },
                {
                  ...
                  type: 'sel',
                  val: newSheet.colors,
                  set: (v) => setNewSheet({ ...newSheet, colors: v }),
                  opts: COLS_OPTS,
                },
                {
                  l: 'Paper Type',
                  type: 'sel',
                  val: newSheet.paper,
                  set: (v) => setNewSheet({ ...newSheet, paper: v }),
                  opts: [
                    'Art 130gsm',
                    'Art 170gsm',
                    'Art 250gsm',
                ...
                },
                {
                  ...
                  val: newSheet.pages,
                  set: (v) =>
                    setNewSheet({ ...newSheet, pages: Number(v) || 1 }),
                },
                {
                  l: 'Rs/Sheet',
                  type: 'num',
                  val: newSheet.rate,
                  set: (v) =>
                    setNewSheet({ ...newSheet, rate: Number(v) || 0 }),
                },
              ].map((f) => (
              ...
              <div style={{ marginBottom: '8px' }}>
                ...
                >
                  Page Trim Size (on {fs(sheetL)}×{fs(sheetH)} paper)
                </div>
                <div style={{ display: 'flex', gap: '6px' }}>
                  <div style={{ flex: 1 }}>
                    <div
                ...
                {(() => {
                  ...
                  return (
                    <div
                      style={{
                        fontSize: '9px',
                        color: f.ups > 0 ? SUC : ERR,
                        fontWeight: '700',
                        marginTop: '4px',
                      }}
                    >
                      {f.ups > 0
                        ? `→ ${f.cols}×${f.rows} = ${f.ups} ups/sheet${
                            f.rot ? ' (rotated)' : ''
                          }`
                        : "⚠ Page doesn't fit this paper"}
                    </div>
                  );
                })()}
              </div>
